<?php

global $config;

include 'template/header.php'

?>

    <div class="d-flex flex-column min-vh-100">
        <main class="flex-fill d-flex mt-5">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-12 col-lg-3">
                        <div class="card mb-4">
                            <img class="card-img-top"
                                 src="https://www.w3schools.com/howto/img_avatar.png"
                                 alt="Card image cap">
                            <div class="card-body">
                                <h4 class="card-title"><?= $config["name-surname"] ?></h4>
                                <hr>
                                <h5 class="card-title"><?= $config["profession"] ?></h5>
                                <p class="card-text"><?= $config["intro-text"] ?><?= $config["intro-text"] ?><?= $config["intro-text"] ?></p>
                            </div>
                        </div>
                    </div>
                    <div class="col">
                        <div class="card">
                            <div class="card-header">
                                About Me
                            </div>
                            <div class="card-body">
                                <blockquote class="blockquote mb-0">
                                    <p><?= $config["intro-text"] ?></p>
                                    <nav class="nav nav-pills nav-justified">

                                        <a href="mailto:<?php echo $config["email"] ?>">
                                            <button type="button" class="btn btn-primary">Contact Me</button>
                                        </a>
                                    </nav>
                                </blockquote>
                            </div>
                        </div>
                        <div class="card-deck text-center mt-4">
                            <div class="card pt-4">
                                <img
                                        class="d-block m-auto"
                                        src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS0RBNEZ1brWx5mD4gh-2XN5utlUmGzstLuGw&usqp=CAU"
                                        alt="Card image cap"
                                        width="64"
                                        height="64">
                                <div class="card-body">
                                    <h5 class="card-title">Python</h5>
                                </div>
                            </div>
                            <div class="card pt-4">
                                <img
                                        class="d-block m-auto"
                                        src="https://miro.medium.com/max/594/1*ymVNbsdd7KxHXHC4-LP7kw.png"
                                        alt="Card image cap"
                                        width="64"
                                        height="64">
                                <div class="card-body">
                                    <h5 class="card-title">C#</h5>
                                </div>
                            </div>
                            <div class="card pt-4">
                                <img
                                        class="d-block m-auto"
                                        src="data:image/png;base64,"
                                        alt="Card image cap"
                                        width="64"
                                        height="64">
                                <div class="card-body">
                                    <h5 class="card-title">Java</h5>
                                </div>
                            </div>
                            <br>
                        </div>

                        <div class="row pt-4">
                            <?php
                            foreach ($config["projects"] as $project) {
                                ?>
                                <div class="col-lg-4 mb-4">
                                    <div class="card">
                                        <a
                                                href="<?= $project['link'] ?>"
                                                target="_blank">
                                            <div class="card-header"><h5 class="mb-0"><?= $project['title'] ?></h5>
                                            </div>
                                        </a>
                                        <div class="card-body">
                                            <?= $project['description'] ?>
                                        </div>
                                    </div>
                                </div>

                                <?php
                            }
                            ?>
                        </div>
                    </div>
                </div>
            </div>
        </main>
        <footer>
            <div class="container d-flex">
                <nav class="w-100 navbar navbar-dark">
                    <div class="navbar-nav m-auto py-3">
                        <div class="content">
                            <a class="facebook" href="https://www.facebook.com/arda.burak.atila"
                               target="_blank"><img alt="facebook-logo" src="assets/img/Facebook.png" width="50px"></a>
                            <a class="github" href="https://github.com/archely" target="_blank"><img
                                        alt="github-logo" src="assets/img/Github.png" width="50px"></a>
                            <a class="linkedin" href="https://www.linkedin.com/in/arda-burak-atila-a5a126141/"
                               target="_blank"><img alt="linkedin-logo" src="assets/img/Linkedin.png" width="50px"></a>

                        </div>
                    </div>
                </nav>
            </div>
        </footer>
    </div>


<?php include 'template/footer.php';